<template>
    ...
    <v-form fast-fail @submit.prevent style="margin-top:100px ;">
      <v-text-field
        ...
        label="Nombre"
      ></v-text-field>
      <v-btn class="mt-2" type="submit" @click="registerEstado" block>Registrar</v-btn>
      <p style="color: red">
        {{ feedback }}
      </p>
    </v-form>
...
const registerEstado = async () => {
  ...
  if (store.user && store.user.rol === "ADMIN") {
    const response = await store.RegisterEstado(
      nombre.value
    );
    ...
    feedback.value = "You don't have permission to register estados.";
  }
};
</script>